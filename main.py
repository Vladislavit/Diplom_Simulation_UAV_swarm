...
def _adapt_to_screen():
    """Масштабує cfg під реальний дисплей якщо вікно не вміщується."""
    info = pygame.display.Info()
    avail_w = info.current_w - 20
    avail_h = info.current_h - 60  # запас для панелі задач і заголовка вікна
    scale = min(avail_w / cfg.WINDOW_WIDTH, avail_h / cfg.WINDOW_HEIGHT, 1.0)
    if scale >= 0.99:
        return
    cfg.WINDOW_WIDTH     = int(cfg.WINDOW_WIDTH     * scale)
    cfg.WINDOW_HEIGHT    = int(cfg.WINDOW_HEIGHT    * scale)
    cfg.PANEL_WIDTH      = int(cfg.PANEL_WIDTH      * scale)
    cfg.MAP_WIDTH        = cfg.WINDOW_WIDTH - cfg.PANEL_WIDTH
    cfg.ZONE_W           = cfg.MAP_WIDTH     // cfg.GRID_COLS
    cfg.ZONE_H           = cfg.WINDOW_HEIGHT // cfg.GRID_ROWS
    cfg.EW_W             = int(cfg.EW_W             * scale)
    cfg.EW_H             = int(cfg.EW_H             * scale)
    cfg.DETECTION_RADIUS = int(cfg.DETECTION_RADIUS * scale)
    cfg.COMM_RADIUS      = int(cfg.COMM_RADIUS      * scale)
    cfg.DRONE_START_X    = int(cfg.DRONE_START_X    * scale)
    cfg.DRONE_START_Y    = int(cfg.DRONE_START_Y    * scale)
    cfg.DRONE_SPACING_X  = int(cfg.DRONE_SPACING_X  * scale)
    cfg.DRONE_SPACING_Y  = int(cfg.DRONE_SPACING_Y  * scale)
    cfg.FOLLOWER_DIST    = int(cfg.FOLLOWER_DIST    * scale)
    cfg.BOIDS_SEP_RADIUS = int(cfg.BOIDS_SEP_RADIUS * scale)
    cfg.DESTROY_DISTANCE = max(8, int(cfg.DESTROY_DISTANCE * scale))
    cfg.DRONE_SPEED      = cfg.DRONE_SPEED * scale
